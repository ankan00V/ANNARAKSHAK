...
| Help inside the app | **Krishi**, always in the bottom-right corner of the farmer app and the sign-in screens: 55 authored help topics covering every screen (en/hi/mr, the other eight from the translation memory) plus live answers from the farmer's own field — can I spray now, will it rain, should I water, what's due today, which pests are coming, what did the expert say. Understands English, Hindi, Marathi and romanised Hinglish; speaks and listens; never makes an answer up and never names a pesticide (`backend/app/krishi.py`, `backend/kb/krishi.json`) |
