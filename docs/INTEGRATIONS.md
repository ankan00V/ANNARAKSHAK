...
These are the next integrations, **not wired yet** — today the code reads only `SARVAM_API_KEY`, `OPENWEATHER_API_KEY`, `ANNRAKSHAK_DATA_DIR`, `ANNRAKSHAK_DB_URL`, `ANNRAKSHAK_VISION`, `VITE_MAP_TILE_URL` and `ANNRAKSHAK_API`. Each row names the variable its adapter will read.
